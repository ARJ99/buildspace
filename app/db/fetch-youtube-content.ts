...
const BATCH_SIZE = 50; // Number of videos to fetch per request
...
function determineDifficulty(title: string): "beginner" | "intermediate" | "advanced" {
    ...
    // explicit keywords
    if (/\b(beginner|intro|introduction|getting started|for beginners)\b/.test(t)) {
        return "beginner";
    }
    if (/\b(intermediate|intermediates|intermediate level)\b/.test(t)) {
        return "intermediate";
    }
    if (/\b(advanced|expert|deep dive|masterclass)\b/.test(t)) {
        return "advanced";
    }

    // fallback heuristic: assume longer/complex titles are intermediate/advanced
    const words = t.split(/\s+/).filter(Boolean).length;
    if (words >= 6) return "intermediate";
    return "beginner";
}
...
async function seedCoursesFromPlaylists() {
    ...
    for (const playlist of coursePlaylists) {
        ...
        // Check if course already exists (filter-object form used by the
